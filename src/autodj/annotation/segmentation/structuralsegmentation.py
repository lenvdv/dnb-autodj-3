...
class StructuralSegmentator:
	...
	def analyse(self, audio_in, downbeats, onset_curve, tempo):
		...
		:param audio_in: The audio, a Numpy array with 1 channel and sample rate 44100
		:param downbeats: Array of downbeats in the audio, as timestamps in seconds.
		Used for aligning detected peaks in the SSM with downbeats in the music.
		:param onset_curve: Onset curve with hop length 512, calculated from the audio. Used to determine the type of
		segment (low or high energy).
		:param tempo: The tempo of the audio in BPM.
		:return: A tuple of arrays (segment_indices, segment_types);
		segment_indices are the downbeat indices of the structural segments.
		segment_types are the type of the segment: "low energy" ('L') or "high energy" ('H').
		'''
		...
		adaptive_mean_odf = adaptive_mean(onset_curve, int((44100*60/tempo)/512) * 4) # -4 dbeats, +4 dbeats
